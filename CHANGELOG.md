...
### 🚀 Добавлено
- **Поддержка текстовой команды `/add_goal`**: Теперь диалог создания цели можно запустить не только кнопкой, но и командой `/add_goal`.
...
- **Проблема с парсингом MarkdownV2**:
  - В `utils/helpers.py` функция `escape_markdown_v2` теперь корректно экранирует `.` и `!`.
  - В `handlers/common.py` и `handlers/task_management.py` сообщения теперь формируются с учетом особенностей MarkdownV2: строковые константы и шаблоны содержат Markdown-разметку как есть, а переменные части экранируются с помощью `escape_markdown_v2` перед подстановкой. Это должно окончательно решить ошибки `BadRequest`.
  - `parse_mode` теперь используется как `ParseMode.MARKDOWN_V2` из `telegram.constants`.
...
- **Ошибка `AttributeError` в `add_goal`**: Исправлена проблема с некорректным присвоением `context.user_data` в обработчиках диалога (`handlers/goals.py`).

### 🚀 Добавлено
- **Логирование сырых ответов LLM**: В `llm/async_client.py` добавлены debug-логи для ответов от OpenAI при генерации плана и мотивации для упрощения дальнейшей отладки.
