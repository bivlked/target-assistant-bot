...
- **Проблема с парсингом MarkdownV2**:
  - В `utils/helpers.py` функция `escape_markdown_v2` теперь корректно экранирует `.` и `!` и другие спецсимволы.
  - В `handlers/common.py`, `handlers/task_management.py` и `handlers/goals.py` (для `show_spreadsheet_link`) сообщения теперь используют `ParseMode.MARKDOWN_V2` и глобальное экранирование всего текста сообщения с помощью `escape_markdown_v2`. Это окончательно решает ошибки `BadRequest`.
